...
void line_clip(int xa, int ya, int xb, int yb, int xmin, int ymin, int xmax, int ymax, int color)
{
    int delta_x, delta_y,dda;
    float x_inc, y_inc, x, y, i;
...
int main()
{
    ...
    printf("Masukkan Jumlah Garis yang akan ada di layar: ");
    scanf("%d",&n);
    ...
    {
        dda_line(koor[i][0],koor[i][1], koor[i+1][0],koor[i+1][1],15);
    }
    ...
    for (int i = 0; i < n*2; i+=2)
    {
        line_clip(koor[i][0],koor[i][1], koor[i+1][0],koor[i+1][1],wx1,wy1,wx2,wy2,2);
    }
